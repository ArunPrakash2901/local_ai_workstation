...
if [ -z "$PROJECT_KEY" ] || [ -z "$TASK_FILE" ]; then
    echo "Usage: ws worktree-create <project_key> <task_file> --dry-run"
    exit 1
fi
...
if [ "$DRY_RUN" != "Yes" ]; then
    set_blocker "BLOCKED_MISSING_DRY_RUN" "This command is preview-only and requires --dry-run."
fi
...
if [ "$DRY_RUN" = "Yes" ] && [ -n "$WSL_PROJECT_PATH" ] && [ -n "$CURRENT_BRANCH" ] && [ -n "$PROPOSED_BRANCH" ] && [ -n "$PROPOSED_WORKTREE_PATH_WSL" ]; then
    BRANCH_COMMAND="git -C \"$WSL_PROJECT_PATH\" branch \"$PROPOSED_BRANCH\" \"$CURRENT_BRANCH\""
...

{
    ...
    echo "- **Dry Run Requested**: $DRY_RUN"
    ...
    if [ "$CLASSIFICATION" = "WORKTREE_CREATE_DRY_RUN_READY" ]; then
        echo "Review the preview only. Actual creation remains disabled."
    else
...
if [ "$CLASSIFICATION" = "WORKTREE_CREATE_DRY_RUN_READY" ]; then
    echo "Next safe action: review the dry-run preview; actual creation remains disabled."
else
