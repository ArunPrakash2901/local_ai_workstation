#!/usr/bin/env python3
"""Temp-root tests for Product Lane implementation plan review dry-run."""
...
from product_implementation_plan_review import (  # noqa: E402
...
def _make_ready_product(root: Path) -> dict[str, object]:
    record = create_product(
        title=f"impl-review sample {uuid4().hex[:8]}",
...
def main() -> int:
    print("Product Implementation Plan Review Validation")
    print("============================================")
    failures: list[str] = []

    temp_parent = _pick_temp_parent(ROOT)
    temp_root = (temp_parent / f"_tmp_product_impl_review_{uuid4().hex}").resolve()
    ...
    try:
        ...
        # Initial review of generated plan (should FAIL due to TODO/UNKNOWN)
        payload = validate_implementation_plan_review_preconditions(temp_root, product_id)
        review_result = review_implementation_plan_text(payload["product_record"], payload["impl_plan_text"], payload_extras=payload)
        expect("initial implementation-plan review fails due to TODOs", review_result["status"] == "FAIL", failures)
        expect("critical_todos detected", len(review_result["critical_todos"]) > 0, failures)

        # Fix TODOs and re-review
        impl_path = pdir / "implementation_plans" / "implementation_plan_v1.md"
        fixed_text = payload["impl_plan_text"].replace("TODO/UNKNOWN", "COMPLETED")
        impl_path.write_text(fixed_text, encoding="utf-8", newline="\n")
        
        # Update hash in product.yaml
        product_file = pdir / "product.yaml"
        ...
        payload_fixed = validate_implementation_plan_review_preconditions(temp_root, product_id)
        review_result_fixed = review_implementation_plan_text(payload_fixed["product_record"], payload_fixed["impl_plan_text"], payload_extras=payload_fixed)
        expect("fixed implementation-plan review passes", review_result_fixed["status"] == "PASS", failures)

        # Missing sections test
        broken_text = fixed_text.replace("## Implementation Phases", "## Deleted Section")
        impl_path.write_text(broken_text, encoding="utf-8", newline="\n")
        product_record["active_implementation_plan_hash"] = __import__("hashlib").sha256((broken_text.rstrip("\n") + "\n").encode("utf-8")).hexdigest()
        product_file.write_text(json.dumps(product_record, indent=2) + "\n", encoding="utf-8")
        
        payload_broken = validate_implementation_plan_review_preconditions(temp_root, product_id)
        review_result_broken = review_implementation_plan_text(payload_broken["product_record"], payload_broken["impl_plan_text"], payload_extras=payload_broken)
        expect("implementation-plan review fails on missing section", review_result_broken["status"] == "FAIL", failures)
        expect("missing_sections detected", "Implementation Phases" in review_result_broken["missing_sections"], failures)

        # Hash mismatch test
        impl_path.write_text(fixed_text + "\nEXTRA", encoding="utf-8")
        try:
            validate_implementation_plan_review_preconditions(temp_root, product_id)
            failures.append("FAIL: implementation-plan review should fail on hash mismatch")
        except ValueError as exc:
            expect("hash mismatch error message", "active_implementation_plan hash mismatch" in str(exc), failures)

        for script_name in ("product_implementation_plan_review.py", "ws_product_implementation_plan_review.py"):
