#!/usr/bin/env python3
"""Temp-root tests for Product Lane design adapter preview dry-run."""
...
from ws_product_design_adapter_preview import main as preview_cli_main  # noqa: E402
...
def _make_product(
    ...
    *,
    ...
    record = create_product(
        title=f"design-preview-{uuid4().hex[:8]}",
...
def _expect_blocked(root: Path, product_id: str, failures: list[str], name: str, expected_fragment: str) -> None:
    try:
        build_design_adapter_preview(root, product_id, "open-design")
...
def main() -> int:
    print("Product Design Adapter Preview Validation")
    print("========================================")
    failures: list[str] = []

    temp_parent = _pick_temp_parent(ROOT)
    temp_root = (temp_parent / f"_tmp_design_adapter_preview_{uuid4().hex}").resolve()
    ...
    try:
        ...
        preview = build_design_adapter_preview(temp_root, ready_product_id, "open-design")
        expect(
            "passes for UI product with approved active PRD, matching active scope, and matching active wireframe",
            preview["adapter_status"] == "READY_FOR_DESIGN_RENDER_PREVIEW",
            failures,
        )
        expect("preview includes planned design run directory", "products/" in preview["planned_design_run_directory"], failures)
        expect(
            "preview includes planned future files",
            all(name in preview["planned_future_files"] for name in PLANNED_FUTURE_FILES),
            failures,
        )
        expect(
            "preview includes forbidden app/source write actions",
            all(item in preview["forbidden_future_actions"] for item in FORBIDDEN_FUTURE_ACTIONS),
            failures,
        )
        expect("preview includes slash command surface /design", preview["slash_command_surface"] == "/design", failures)
        expect(
            "preview includes canonical mapped ws command",
            "ws product-design-adapter-preview --product" in preview["canonical_ws_command"],
            failures,
        )
        ...
        _ = build_design_adapter_preview(temp_root, ready_product_id, "open-design")
        ...
        unsupported_id, unsupported_dir = _make_product(temp_root, product_type="automation")
        _expect_blocked(temp_root, unsupported_id, failures, "blocks unsupported product type if current product type is not UI-capable", "not UI-capable")
        _ = unsupported_dir
        ...
        preview_with_tech = build_design_adapter_preview(temp_root, with_tech_id, "open-design")
        expect(
        ...
        expect("CLI requires --dry-run", rc == 2 and "Write-mode product-design-adapter-preview is not implemented. Use --dry-run." in stdout_capture.getvalue(), failures)
        ...
        for script_name in ("product_design_adapter.py", "ws_product_design_adapter_preview.py"):
            ...
            expect(
                "no external-adapter workflow execution",
