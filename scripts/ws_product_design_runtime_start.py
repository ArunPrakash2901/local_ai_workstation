#!/usr/bin/env python3
"""Dry-run or confirm Open Design managed runtime start."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preview or start Open Design managed runtime through pnpm tools-dev."
    )
    parser.add_argument("--tool", help="Design adapter tool id (open-design in this slice).")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--dry-run", action="store_true", help="Preview only; no writes and no runtime start.")
    group.add_argument("--confirm", action="store_true", help="Start managed runtime and write local capture metadata.")
    parser.add_argument(
        "--root",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.dry_run:
            ...
            print(render_managed_runtime_plan(plan, action="start dry-run").rstrip())
            return 0

        result = execute_managed_runtime_start(root, tool, confirm=True)
