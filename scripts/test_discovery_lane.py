...
def main() -> int:
    ...
    if existing_ingest.exists():
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            rc = dispatcher.main(["approve-set", "examples", "--dry-run"])
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            rc = dispatcher.main(["handoff-list"])
        assert_true(rc == 0, "dispatcher handoff-list failed")
        with contextlib.redirect_stdout(io.StringIO()):
            rc = dispatcher.main(["queue-plan", "examples"])
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        rc = dispatcher.main(["queue-plan", positive_set_id, "--write-report"])
