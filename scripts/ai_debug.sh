...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        *) 
            ...
            elif [ -z "$ERROR_LOG_PATH" ]; then ERROR_LOG_PATH="$1"; 
            fi 
            ;;
    esac
    shift
...
if [ -z "$PROJECT_KEY" ] || [ -z "$ERROR_LOG_PATH" ]; then
    echo "Usage: ai_debug <project_key> <error_log_path> [--profile <p>] [--model <m>] [--kv <k>] [--context <c>]"
    exit 1
fi
...
RUN_DIR="$RUNS_DIR/${TIMESTAMP}_debug_${PROJECT_KEY}"
...
if [ -f "$GRAPH_PATH" ]; then
    ...
    source "$VENV_PATH"
    graphify query "debug error: $ERROR_LOG" --graph "$GRAPH_PATH" > "$RUN_DIR/graph_context.md"
    deactivate
else
...
DEBUG_TEMPLATE=$(cat "/mnt/d/_ai_brain/prompts/local_debugger.md")
...
print(template.replace('{{project_key}}', project_key).replace('{{error_log}}', error_log).replace('{{graph_context}}', graph_context))
" "$DEBUG_TEMPLATE" "$PROJECT_KEY" "$ERROR_LOG" "$(cat "$RUN_DIR/graph_context.md")")
...
echo "Debugging with $MODEL (Profile: $PROF, KV: $KV_PROF, CTX: $CTX_SIZE)..."
